...
def generate_move_cache():
	...
	# 			moves[squareKey] = [Bitboard([m]) for m in allMoves]
	# 			moveSets[squareKey] = Bitboard(allMoves)
	# 		return moves,moveSets
	#
	# moves,moveSets = [],[]
	# for pieceType in [PAWN, KNIGHT, BISHOP, ROOK, QUEEN, KING]:
	# 	bitboards = Bitboards_for_piece_type(pieceType)
